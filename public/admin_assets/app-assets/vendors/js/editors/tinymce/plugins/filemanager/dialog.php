...
if (isset($_POST['submit'])) {
	...
	if (isset($_GET['subfolder']) && !empty($_GET['subfolder'])) {
		...
		$thumbs_path = $thumbs_path . $subfolder . '/';
					...
					<?php } else { ?>
						...
						<ul class="grid cs-style-2">
							<?php
									...
									<li>
										<figure>
											<a title="<?php echo lang_Open ?>" href="dialog.php?type=<?php echo $_GET['type'] ?>&subfolder=<?php echo $subfolder ?>&editor=<?php echo $_GET['editor'] ? $_GET['editor'] : 'mce_0'; ?>&popup=<?php echo $popup; ?>&field_id=<?php echo $_GET['field_id'] ? $_GET['field_id'] : ''; ?>&lang=<?php echo $_GET['lang'] ? $_GET['lang'] : 'en_EN'; ?>&fldr=<?php echo $src ?>&<?php echo uniqid() ?>">
												<?php if ($file == "..") { ?>
										...
										<?php } else { ?>
											...
											<figcaption>
												<a href="javascript:void('')" class="erase-button" <?php if ($delete_folder) { ?>onclick="if(confirm('<?php echo lang_Confirm_Folder_del; ?>')){ delete_folder('<?php echo $root . $cur_dir . $file; ?>','<?php echo $thumbs_path . $subdir . $file; ?>'); $(this).parent().parent().parent().hide(200); return false;}" <?php } ?> title="<?php echo lang_Erase ?>">
												...
												if ($file != '.' && $file != '..' && !is_dir($root . $cur_dir . $file)) {
													...
													if (in_array($file_ext, $ext)) {
														if (in_array($file_ext, $ext_img)) {
															...
															$thumb_file_path = $thumbs_path . $subdir . $file;
															if (!file_exists($thumb_file_path))
																create_img_gd(dirname(__FILE__) . DIRECTORY_SEPARATOR . $current_path . $subfolder . DIRECTORY_SEPARATOR . $subdir . $file, $thumb_file_path, 122, 91);
												...
												<figure>
													...
													<figcaption>
														<form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
															...
															<?php } ?>
															<a href="javascript:void('');" class="erase-button" <?php if ($delete_file) { ?>onclick=" if(confirm('<?php echo lang_Confirm_del; ?>')){ delete_file('<?php echo $root . $cur_dir . $file; ?>','<?php echo $thumbs_path . $subdir . $file; ?>'); $(this).parent().parent().parent().parent().hide(200); return false;}" <?php } ?> title="<?php echo lang_Erase ?>"><i class="icon-trash <?php if (!$delete_file) echo 'icon-white'; ?>"></i></a>
														</form>
													</figcaption>
												</figure>
